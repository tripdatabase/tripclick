...
relevance_ranges = [(0.0,0.04),(0.04,0.3),(0.3,1.0),(1.0,1+1e-5)]

def get_relevance_score(ct_value):
    for i, _range in enumerate(relevance_ranges):
        if ct_value >= _range[0] and ct_value < _range[1]:
            return i
...
## Loading ctmodel
_path = os.path.join(args.origindatadir, 'qry_doc_dctr.pkl')
...
for _tuple in ctmodel_tuples:
    ...
    for _docid in _tuple[1].keys():
        if _docid not in docs:
            continue
        _values[_docid] = get_relevance_score(_tuple[1][_docid])
    if len(_values.keys()) > 0:
        if sum(_values.values()) > 0:
            ctmodel_tuples_filtered.append((_qid, _values))

## Saving Files
_path = os.path.join(args.colldir, 'QRels/qrels.dctr.all.txt')
